...
const statefulHookNames = Object.values(STATEFUL_HOOK_NAMES);
const NON_USER_COMPONENT_NAMES = ['Batcher'];
export function checkIfFiberNodeIsUserComponent(fiber) {
    return !NON_USER_COMPONENT_NAMES.includes(fiber.elementType.name);
}
export function checkIfFiberNodeHasState(fiber) {
...
export function checkIfFiberNodeStateIsRecoilState(nextState) {
    if (typeof nextState === 'object' && nextState.baseState === null) {
        return true;
    }
    return false;
}
export function extractLocalStatesFromFiberNode(fiber) {
    const states = [];
